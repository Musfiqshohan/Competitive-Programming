...
ll f(int idx, int k, bool par_flag, bool dan_flag)
{
    ...
    if(dan_flag==true)
    {
        for(int x=0; x<k; x++)
        {
            ret= add(ret, mul( f(fc[idx],x, true, false), f(sib[idx], (k-1)-x,  par_flag, false) ));  ///i

        }

        for(int x=0; x<=k; x++)
        {
            ret= add(ret, mul( f(fc[idx], x, false, false), f(sib[idx], k-x, par_flag, dan_flag) )); ///ii
        }
    }
    else
    {
        if(par_flag==true)
        {
            ...
            {
                ret= add(ret, mul( f(fc[idx], x, true, false), f(sib[idx], k-1-x, par_flag, false)  ));
            }
        ...
        else
        {
            for(int x=0; x<k; x++)/// will color
            {
                ret= add(ret, mul( f(fc[idx], x, true, true), f(sib[idx], k-1-x, par_flag, false)  ));
            }
